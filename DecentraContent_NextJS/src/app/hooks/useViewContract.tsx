...
const useViewContract = () => {
  ...
  const callViewContractFunction = async (
    ...
    if (!loading && contract && accountDetails.account) { // Check if not loading
      try {
        const method = isViewFunction? 'call' : 'send';
        const options = { from: accountDetails.account, value: value || '0' } as { from?: string; value?: string };
        if (method === 'call') {
          delete options.from;
        }
        const result = await contract.methods[functionName](...(params || [])).call(options);
